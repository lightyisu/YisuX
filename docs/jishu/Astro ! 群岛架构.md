...
不过我发现这个几个游戏网站都是重CSR渲染的，也许文字内容不重要？只需要meta上的做SEO就可以。而腾讯的游戏网站会把完整的HTML作为渲染响应。但是奇怪的是最应该做SEO的wegame基本上一点没做，而是完全CSR的。在百度和bing还好，在Google就完全wegame+游戏名找查无此人（可能本来就不国际化）。Gemini说腾讯会针对爬虫条件性给响应的可能吧，现在用CSR可能就够了。
